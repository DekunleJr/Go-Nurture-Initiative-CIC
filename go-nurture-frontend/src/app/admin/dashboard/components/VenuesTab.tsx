...
export default function VenuesTab({ venues, onCreateVenue, onUpdateVenue, onDeleteVenue, onRefresh }: VenuesTabProps) {
  ...
  // Load leaflet only in the browser (avoids SSR window errors)
  useEffect(() => {
    let cancelled = false;
    import("leaflet").then((mod) => {
      if (!cancelled) {
        setLeafletLib(mod.default as typeof L);
      }
    });
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    const icon = leafletLib.divIcon({
      ...
      html: '<div class="venue-marker-pin">' +
  ...
  const geocodeAddress = async () => {
    if (!address || !city) {
      toast.error("Please fill in address and city first");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(
        ...
        { headers: { "User-Agent": "GoNurtureInitiativeCIC/1.0" } }
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingVenue) {
        await onUpdateVenue(editingVenue.id, venueData);
          ...
          <div className="mt-4 flex items-end gap-3">
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500 mb-2">
                Search the address to place a pin on the map. The venue will be geocoded automatically if you skip this.
              </p>
              <button
                type="button"
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="inline-flex items-center justify-center p-1 text-(--color-accent) hover:bg-(--color-accent)/10 rounded-lg"
                      title="Edit venue"
                    >
                      <Pencil size={16} />
                    </button>
